<?php
class ControllerShippingOmnivalt extends Controller {
	private $error = array();
   ...
   // return $this->response->setOutput(json_encode($order.'ok'.$labelsCount));
	...
	public function index() {
      ...
      if (!empty($this->request->post['download'])) 
        $this->fetchUpdates();
      		...
      		$this->redirect($this->url->link('extension/shipping', 'token=' . $this->session->data['token'], true));
    ...
    foreach($sender_array as $key){
      ...
      'separator' => false
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('shipping/omnivalt', 'token=' . $this->session->data['token'], true),
      'separator' => false

		);

		$this->data['action'] = $this->url->link('shipping/omnivalt', 'token=' . $this->session->data['token'], true);
    ...
    foreach($sender_array as $key){
      if (isset($this->request->post['omnivalt_'.$key])) {
        $this->data['omnivalt_'.$key] = $this->request->post['omnivalt_'.$key];
      } else {
        $this->data['omnivalt_'.$key] = $this->config->get('omnivalt_'.$key);
      }
    }
    
    if (isset($this->request->post['omnivalt_url'])) {
			$this->data['omnivalt_url'] = $this->request->post['omnivalt_url'];
		} else {
    ...
    if ($this->data['omnivalt_url'] == '') {
      $this->data['omnivalt_url'] = 'https://edixml.post.ee';
		...
		if (isset($this->request->post['omnivalt_user'])) {
			$this->data['omnivalt_user'] = $this->request->post['omnivalt_user'];
		} else {
			$this->data['omnivalt_user'] = $this->config->get('omnivalt_user');
		}

		if (isset($this->request->post['omnivalt_password'])) {
			$this->data['omnivalt_password'] = $this->request->post['omnivalt_password'];
		} else {
			$this->data['omnivalt_password'] = $this->config->get('omnivalt_password');
		}
    
		if (isset($this->request->post['omnivalt_service'])) {
			$this->data['omnivalt_service'] = $this->request->post['omnivalt_service'];
		} elseif ($this->config->has('omnivalt_service')) {
			$this->data['omnivalt_service'] = $this->config->get('omnivalt_service');
		} else {
			$this->data['omnivalt_service'] = array();
		}
    ...
    if (isset($this->request->post['omnivalt_parcel_terminal_price'])) {
			$this->data['omnivalt_parcel_terminal_price'] = $this->request->post['omnivalt_parcel_terminal_price'];
		} else {
			$this->data['omnivalt_parcel_terminal_price'] = $this->config->get('omnivalt_parcel_terminal_price');
    }
    if (isset($this->request->post['omnivalt_courier_price'])) {
			$this->data['omnivalt_courier_price'] = $this->request->post['omnivalt_courier_price'];
		} else {
			$this->data['omnivalt_courier_price'] = $this->config->get('omnivalt_courier_price');
		}
    //Additions for Latvia
    if (isset($this->request->post['omnivalt_parcel_terminal_pricelv'])) {
          $this->data['omnivalt_parcel_terminal_pricelv'] = $this->request->post['omnivalt_parcel_terminal_pricelv'];
   } else {
          $this->data['omnivalt_parcel_terminal_pricelv'] = $this->config->get('omnivalt_parcel_terminal_pricelv');
  }
  if (isset($this->request->post['omnivalt_courier_pricelv'])) {
    $this->data['omnivalt_courier_pricelv'] = $this->request->post['omnivalt_courier_pricelv'];
  } else {
    $this->data['omnivalt_courier_pricelv'] = $this->config->get('omnivalt_courier_pricelv');
  }
  //Additions for Estonia
 if (isset($this->request->post['omnivalt_parcel_terminal_priceee'])) {
          $this->data['omnivalt_parcel_terminal_priceee'] = $this->request->post['omnivalt_parcel_terminal_priceee'];   } else {
          $this->data['omnivalt_parcel_terminal_priceee'] = $this->config->get('omnivalt_parcel_terminal_priceee');
   }

   if (isset($this->request->post['omnivalt_courier_priceee'])) {
    $this->data['omnivalt_courier_priceee'] = $this->request->post['omnivalt_courier_priceee'];
  } else {
    $this->data['omnivalt_courier_priceee'] = $this->config->get('omnivalt_courier_priceee');
  }
/** End of pricing */
    if (isset($this->request->post['omnivalt_company'])) {
			$this->data['omnivalt_company'] = $this->request->post['omnivalt_company'];
		} else {
			$this->data['omnivalt_company'] = $this->config->get('omnivalt_company');
		}
    
    if (isset($this->request->post['omnivalt_bankaccount'])) {
			$this->data['omnivalt_bankaccount'] = $this->request->post['omnivalt_bankaccount'];
		} else {
			$this->data['omnivalt_bankaccount'] = $this->config->get('omnivalt_bankaccount');
		}
    
    if (isset($this->request->post['omnivalt_pickupstart'])) {
			$this->data['omnivalt_pickupstart'] = $this->request->post['omnivalt_pickupstart'];
		} else {
			$this->data['omnivalt_pickupstart'] = $this->config->get('omnivalt_pickupstart');
		}
    if ($this->data['omnivalt_pickupstart'] == '') {
      $this->data['omnivalt_pickupstart'] = "8:00";
    }
    
    if (isset($this->request->post['omnivalt_pickupfinish'])) {
			$this->data['omnivalt_pickupfinish'] = $this->request->post['omnivalt_pickupfinish'];
		} else {
			$this->data['omnivalt_pickupfinish'] = $this->config->get('omnivalt_pickupfinish');
		}
    if ($this->data['omnivalt_pickupfinish'] == '') {
      $this->data['omnivalt_pickupfinish'] = "17:00";
    }
    
    if (isset($this->request->post['omnivalt_cod'])) {
			$this->data['omnivalt_cod'] = $this->request->post['omnivalt_cod'];
		} else {
			$this->data['omnivalt_cod'] = $this->config->get('omnivalt_cod');
		}
    
		if (isset($this->request->post['omnivalt_pickup_type'])) {
			$this->data['omnivalt_pickup_type'] = $this->request->post['omnivalt_pickup_type'];
		} else {
			$this->data['omnivalt_pickup_type'] = $this->config->get('omnivalt_pickup_type');
		}
    
    if (isset($this->request->post['omnivalt_status'])) {
			$this->data['omnivalt_status'] = $this->request->post['omnivalt_status'];
		} else {
			$this->data['omnivalt_status'] = $this->config->get('omnivalt_status');
		}
    
		if (isset($this->request->post['omnivalt_sort_order'])) {
			$this->data['omnivalt_sort_order'] = $this->request->post['omnivalt_sort_order'];
		} else {
			$this->data['omnivalt_sort_order'] = $this->config->get('omnivalt_sort_order');
		}
    $this->data['omnivalt_terminals'] = $this->loadTerminals();
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'shipping/omnivalt')) {
  ...
  protected static function getReferenceNumber($order_number, $weight = 1)
  {
              ...
              <xsd:businessToClientMsgRequest>
                 ...
                 <interchange msg_type="info11">
                    ...
                    <item_list>
                      ...
                      for ($i = 0; $i <$packs; $i++):
                      $preg = preg_match('/\d+/', $order['shipping_postcode'], $matches);//426r    <address postcode="'.$order['shipping_postcode'].'"
                      $postcode = $preg?  $matches[0] : '';
                      $xmlRequest .= '
  ...
  private function getShipmentLabels($barcodes,$order_id=0)
  {
    $errors = array();
        ...
        $xmlResponse = str_ireplace(array('SOAP-ENV:', 'SOAP:'), '', $xmlResponse);
        $xml = simplexml_load_string($xmlResponse);
        if (!is_object($xml)) {
  ...
  public function parseXmlTrackingResponse($trackings, $response)
  {
    $errors = array();
        ...
        /*
        if (!empty($resultArr)) {
            foreach ($resultArr as $trackNum => $this->data) {
                $tracking = $this->_trackStatusFactory->create();
                $tracking->setCarrier($this->_code);
                $tracking->setCarrierTitle($this->getConfigData('title'));
                $tracking->setTracking($trackNum);
                $tracking->addData($this->data);
...
//die('fsdfsd');
  ...
  private function readyStatus(){
    ...
    $status_id = $this->config->get('omnivalt_status_id');
...
//var_dump($_POST);exit();
   ...
   public function manifest()
   {
        ...
        foreach ($_POST['selected'] as $order_id){
          ...
          //setting package weight
           ...
           if($productsCnt == 0) $productsCnt = 1;
  ...
  {
     $labelsCount =  intval($this->request->post['labelsCount']);
     $omnivaWeight = floatval($this->request->post['omnivaWeight']);
     $cod_available = intval($this->request->post['cod_available']);
     $cod_value = floatval($this->request->post['cod_value']);
     $delivery_method = $this->request->post['delivery_method'];
     $order_id = intval($this->request->post['order_id']);

 //var_dump($labelsCount, $omnivaWeight,  $delivery_method,  $order_id,  $cod_available,  $cod_value); 
...
$this->redirect($this->url->link('sale/order/info', 'token=' . $this->session->data['token'].  '&order_id=' . $order_id, true));
